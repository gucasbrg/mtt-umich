...
if [ $OBJ -eq 0 ]; then
	...
	CONFLIST=$ROOTDIR$SEQNAME"_nec_conflist.txt"
	OBJ_PARAMS="--objtype Car\
				--min_height 0.8 --max_height 1.8 --det_threshold -1.0 --det_weight .75"
	POSTFIX=car.$POSTFIX
fi

rm -rf results/$SEQNAME.$POSTFIX
mkdir results/$SEQNAME.$POSTFIX
INIT_CAM="--init_cam_focal 980.0 --init_cam_x 0 --init_cam_y 1.3 --init_cam_z 0.0 --init_cam_v 5.0 --init_cam_yaw 0.0 --init_cam_horizon $HOR --init_cam_xcenter 320"
PARAMS="--outdir results/$SEQNAME.$POSTFIX\
		--outvid results/$SEQNAME.$POSTFIX\
		--showimg false\
		--mean_horizon 250 --std_horizon 10\
		--prob_stay 0.9 --prob_enter 0.2\
		--cam_motion_std_horizon 100 --cam_sample_std_horizon 100 --cam_motion_std_yaw 0.6 --cam_sample_std_yaw 0.05 --cam_motion_std_v 5 --cam_sample_std_v 1\
		...
		--num_samples 12000 --num_burnin 2000 --num_thinning 200"
